  ...
  "For-comprehension over result" should {
    ...
    "fail with sub-parser when there is no data" >> {
      ...
      "with captured failure" in withQueryResult("scalar") { implicit c =>
        ...
        SQL("SELECT * FROM test").asTry(parser.single) must beFailedTry.withThrowable[AnormException](
          ".*'col1' not found.*"
        )
      }
    }
  ...
  "Column value" should {
    ...
    "be found by alias when column name is duplicated" in {
      withH2Database { implicit c =>
        ...
        SQL"insert into test1(id, foo, bar) values ($id1, ${"Lorem"}, ${100})".execute()

        val id2 = System.identityHashCode(id1).toLong
        SQL"insert into test1(id, foo, bar) values ($id2, ${"Ipsum"}, ${101})".execute()

        SQL"""SELECT a.foo AS ali, b.foo, b.foo AS ias FROM test1 a
              JOIN test1 b ON a.bar <> b.bar WHERE a.id = $id1 LIMIT 1""".as(
          (SqlParser.str("ali") ~ SqlParser.str("foo") ~ SqlParser.str("ias")).map(SqlParser.flatten).single
        ) must_=== (("Lorem", "Ipsum", "Ipsum"))
      }
    }
    ...
    "be found by user alias" in withTestDB(v2) { implicit c =>
      (SQL"SELECT foo AS AL, bar FROM test1"
        ...
        SQL"SELECT foo AS AL, bar FROM test1"
          .asTry(SqlParser.str("pre.AL").single, ColumnAliaser.withPattern(Set(1), "pre."))
  ...
  def withTestDB[T](foo: String)(f: java.sql.Connection => T): T =
    withH2Database { implicit c =>
      ...
      SQL("insert into test1(id, foo, bar) values ({id}, {foo}, {bar})")
        .on(Symbol("id") -> 10L, Symbol("foo") -> foo, Symbol("bar") -> 20)
        .execute()

      f(c)
    }

  def queryResultAndOptions[A](r: QueryResult, ps: List[(String, String)] = List.empty)(
      f: java.sql.Connection => A
  ): A = f(connection(handleQuery { _ => r }, ps: _*))
}
