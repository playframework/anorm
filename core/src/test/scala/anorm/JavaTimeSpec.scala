  ...
  "Metadata" should {
    "be provided for parameter" >> {
      s"of type Instant" in {
        Option(implicitly[ParameterMetaData[Instant]].sqlType).aka("SQL type") must beSome
      }

      s"of type LocalDateTime" in {
        Option(implicitly[ParameterMetaData[LocalDateTime]].sqlType).aka("SQL type") must beSome
      }

      s"of type LocalDate" in {
        Option(implicitly[ParameterMetaData[LocalDate]].sqlType).aka("SQL type") must beSome
      }
...
object JavaTimeParameterSpec extends Specification {
  ...
  def withJavaTimeConnection[A](ps: (String, String)*)(f: java.sql.Connection => A): A = f(
    connection(
      handleStatement.withUpdateHandler {
        case UpdateExecution("set-instant ?", DParam(t: java.sql.Timestamp, SqlTimestamp) :: Nil)
            if t.getTime == 123456789000L =>
          1 /* case ok */
        case UpdateExecution("set-null-instant ?", DParam(null, SqlTimestamp) :: Nil) => 1 /* case ok */
        case UpdateExecution("set-local-date-time ?", DParam(t: java.sql.Timestamp, SqlTimestamp) :: Nil)
            if t.getTime == LocalDateTime1Epoch =>
          1 /* case ok */
        case UpdateExecution("set-null-local-date-time ?", DParam(null, SqlTimestamp) :: Nil) => 1 /* case ok */
        case UpdateExecution("set-local-date ?", DParam(t: java.sql.Timestamp, SqlTimestamp) :: Nil)
            if t.getTime == LocalDate1Epoch =>
          1 /* case ok */
        case UpdateExecution("set-null-local-date ?", DParam(null, SqlTimestamp) :: Nil) => 1 /* case ok */
        case UpdateExecution("set-zoned-date-time ?", DParam(t: java.sql.Timestamp, SqlTimestamp) :: Nil)
            if t.getTime == 123456789000L =>
          1 /* case ok */
  ...
  "Java time named parameters" should {
    ...
    "be null instant" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-instant {p}").on("p" -> null.asInstanceOf[Instant]).execute() must beFalse
    }

    "be undefined instant" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-instant {p}").on("p" -> Option.empty[Instant]).execute() must beFalse
    }

    "be local date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-local-date-time {p}").on("p" -> LocalDateTime1).execute() must beFalse
    }

    "be null local date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-local-date-time {p}").on("p" -> null.asInstanceOf[LocalDateTime]).execute() must beFalse
    }

    "be undefined local date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-local-date-time {p}").on("p" -> Option.empty[LocalDateTime]).execute() must beFalse
    }
    ...
    "be null local date" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-local-date {p}").on("p" -> null.asInstanceOf[LocalDate]).execute() must beFalse
    }

    "be undefined local date" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-local-date {p}").on("p" -> Option.empty[LocalDate]).execute() must beFalse
    }

    "be zoned date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-zoned-date-time {p}").on("p" -> ZonedDateTime1).execute() must beFalse
    }

    "be null zoned date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-null-zoned-date-time {p}").on("p" -> null.asInstanceOf[ZonedDateTime]).execute() must beFalse
    }
  ...
  "Java time parameter in order" should {
    ...
    "be one local date/time" in withJavaTimeConnection() { implicit c =>
      SQL("set-local-date-time {p}").onParams(pv(LocalDateTime1)).execute() must beFalse
    }

    "be one local date" in withJavaTimeConnection() { implicit c =>
      SQL("set-local-date {p}").onParams(pv(LocalDate1)).execute() must beFalse
    }
