...
final class AnormSpec extends Specification with H2Database with AnormTest {
  ...
  "Row parser" should {
    ...
    "handle scalar result" >> {
      ...
      "return None for missing optional value" in withQueryResult(null.asInstanceOf[String]) { implicit c =>
        SQL"SELECT * FROM test".withFetchSize(Some(1)).as(scalar[String].singleOpt) must beNone
      }

      "return 0 for missing optional numeric" in withQueryResult(null.asInstanceOf[Double]) { implicit c =>
        SQL("SELECT * FROM test").as(scalar[Double].singleOpt).aka("single value") must beSome(0d)

      }

      "throw exception when single result is missing" in withQueryResult(fooBarTable) { implicit c =>
      ...
      "return single string from executed query" in withQueryResult("Result for test-proc-1") { implicit c =>
    ...
    "handle optional property in case class" >> {
      "return instance with defined option" in withQueryResult(
        rowList2(classOf[Int] -> "id", classOf[String] -> "val") :+ (2, "str")
      ) { implicit c =>
      ...
      "throw exception when type doesn't match" in withQueryResult(fooBarTable :+ (1L, "str", 3)) { implicit c =>
    ...
    "throw exception when type doesn't match" in withQueryResult("str") { implicit c =>
    ...
    lazy val rows2 = rowList2(classOf[Int] -> "id", classOf[String] -> "val") :+ (2, "str")

    "check column is found on left" >> {
      "successfully with mandatory value" in withQueryResult(rows2) { implicit c =>
        SQL("SELECT * FROM test")
      ...
      "successfully with optional value" in withQueryResult(rows1) { implicit c =>
        SQL("SELECT * FROM test")
          .as((SqlParser.int("id").? ~> SqlParser.str("val")).single)
          .aka("mapped data") must_=== "str"
    ...
    "fold row" in withQueryResult(rows2) { implicit c =>
      SQL("SELECT * FROM test").as(
        SqlParser
          ...
          .singleOpt
      ) must beSome(("str", ".val", "java.lang.String") :: (2, ".id", "int") :: Nil)
    }
  }

  "Instance of case class" should {
    "be parsed using convience parsers with column names" in withH2Database { implicit c =>
      createTest1Table()
    ...
    "be parsed using raw 'get' parser with column names" in withQueryResult(fooBarTable :+ (11L, "World", 21)) {
      implicit c =>
        SQL("select * from test1 where id = {id}")
          .on(Symbol("id") -> 11L)
  ...
  "Result mixing named and unnamed columns" should {
    "be parsable using named and positional parsers" in withQueryResult(
      rowList3(classOf[String], classOf[String], classOf[String]).withLabel(2, "named") :+ ("a", "b", "c")
    ) { implicit con =>
  ...
  "List" should {
    "be Nil when there is no result" in withQueryResult(QueryResult.Nil) { implicit c =>
    ...
    "be parsed from mapped result" in withQueryResult(
      rowList2(classOf[String] -> "foo", classOf[Int] -> "bar").append("row1", 100) :+ ("row2", 200)
    ) { implicit c =>
    ...
    "be parsed from class mapping" in withQueryResult(fooBarTable :+ (12L, "World", 101) :+ (14L, "Mondo", 3210)) {
      implicit c =>
        val exp =
    ...
    "be parsed from mapping with optional column" in withQueryResult(
      rowList2(classOf[Int] -> "id", classOf[String] -> "val").append(9, null.asInstanceOf[String]) :+ (2, "str")
    ) { implicit c =>
    ...
    "include scalar values" in withQueryResult(stringList :+ "A" :+ "B" :+ "C" :+ "D") { implicit c =>
  ...
  "Aggregation over all rows" should {
    "be empty when there is no result" in withQueryResult(QueryResult.Nil) { implicit c =>
      SQL"EXEC test"
  ...
  "Aggregation over variable number of rows" should {
    ...
    "stop after first row" in withQueryResult(rowList1(classOf[String] -> "foo") :+ "A" :+ "B") { implicit c =>
      var i = 0
