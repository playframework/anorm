...
class StatementParserSpec extends org.specs2.mutable.Specification {
  ...
  "Statement" should {
    "be parsed with 'name' and 'cat' parameters and support multiple lines" in {
      ...
      """)
        ...
        TokenizedStatement(
          List(
            TokenGroup(
              List(StringToken("""SELECT *, '^foo' as foo, 2^3 as bar FROM schema.table
        -- Foo's comment
        WHERE (name = """)),
              Some("name")
            ),
            TokenGroup(List(StringToken(" AND category = ")), Some("cat")),
            TokenGroup(
              List(StringToken(""") OR id = ?
      """)),
              None
            )
          ),
    ...
    "detect missing query parameter" in withQueryResult(stringList :+ "test") { implicit con =>
      SQL("SELECT name FROM t WHERE id = {id}").execute().aka("query") must throwA[java.util.NoSuchElementException]
    }

    "reserved '%' character must be preserved as special token" in {
      SqlStatementParser
        .parse("SELECT * FROM Test WHERE id = {id} AND n LIKE '%strange%s fruit%s'")
        .aka("statement") must beSuccessfulTry(
        TokenizedStatement(
          List(
            TokenGroup(List(StringToken("SELECT * FROM Test WHERE id = ")), Some("id")),
            TokenGroup(
              List(
                StringToken(" AND n LIKE '"),
                PercentToken,
                StringToken("strange"),
                PercentToken,
                StringToken("s fruit"),
    ...
    "characters must be escaped by '\\'" in {
      SqlStatementParser
        .parse("SELECT \\* \\FROM Test WHERE title = 'x \\{foo\\}' AND id = {id}")
        .aka("parsed") must beSuccessfulTry(
        TokenizedStatement(
          List(TokenGroup(List(StringToken("SELECT * FROM Test WHERE title = 'x {foo}' AND id = ")), Some("id"))),
          List("id")
        )
      )
  ...
  "Tokenized statement" should {
    "return some prepared query with updated statement" in {
      val stmt1 = TokenizedStatement(
        List(
          TokenGroup(List(StringToken("SELECT * FROM t WHERE c IN (")), Some("cs")),
          TokenGroup(List(StringToken(") AND id = ")), Some("id"))
        ),
        List("cs", "id")
      )

      val stmt2 = TokenizedStatement(
        List(
          TokenGroup(
            List(StringToken("SELECT * FROM t WHERE c IN ("), StringToken("?, ?"), StringToken(") AND id = ")),
            Some("id")
          )
        ),
        List("cs_1", "cs_2", "id")
      ...
      val stmt3 = TokenizedStatement(
        List(
          TokenGroup(
            List(
              StringToken("SELECT * FROM t WHERE c IN ("),
              StringToken("?, ?"),
              StringToken(") AND id = "),
              StringToken("?")
            ),
    ...
    val stmt = TokenizedStatement(
      List(
        TokenGroup(
          List(
            StringToken("SELECT * FROM name LIKE "),
            StringToken("'"),
            PercentToken,
            StringToken("strange"),
            StringToken("'"),
  ...
  "String interpolation" should {
    "be successfully prepared" in {
      ...
      (query.sql.stmt.aka("tokenized") must_=== TokenizedStatement(
        List(
          TokenGroup(
            List(StringToken("""
         SELECT * FROM (SELECT 'Hello' AS COL1, 'World' AS COL2) AS MY_TABLE WHERE COL1 LIKE """)),
            Some("_0")
          ),
          TokenGroup(
            List(StringToken(""" + '{foo} %'
      """)),
            None
          )
        ),
        ...
        query.params.aka("parameters") must_=== Map[String, ParameterValue]("_0" -> "sinki")
      }
    }
    ...
    "handle values as '#' escaped part in statement or SQL parameters" in {
      ...
      (query.sql.stmt.aka("tokenized statement") must_=== TokenizedStatement(
        List(
          TokenGroup(
            List(
              StringToken("SELECT"),
              ...
              StringToken(" "),
              StringToken("Test"),
              StringToken(" WHERE id = ")
            ),
            Some("_0")
          ),
          TokenGroup(List(StringToken(" AND code IN (")), Some("_1")),
          TokenGroup(List(StringToken(")")), None)
        ),
        List("_0", "_1")
      )).and(query.as(SqlParser.scalar[String].single) must_=== "ok")
    }
  }
}
