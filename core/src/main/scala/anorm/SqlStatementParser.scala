  ...
  private val instr: Parser[TokenizedStatement] = {
    ...
    ("[ \r\n\t]*".r ~> rep(
      ...
      _.foldLeft(List.empty[TokenGroup] -> List.empty[String]) {
        case ((TokenGroup(ts, None) :: groups, ns), Some(TokenGroup(Nil, Some(n)))) =>
      ...
      case (TokenGroup(ts, pl) :: gs, ns)
          if // trim end #2
          ts.lastOption.filter(_ == StringToken(" ")).isDefined =>
        TokenizedStatement((TokenGroup(ts.dropRight(1), pl) :: gs).reverse, ns.reverse)
